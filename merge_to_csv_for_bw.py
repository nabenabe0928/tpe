...
if __name__ == "__main__":
    dir_names = [d for d in os.listdir("results-bandwidth/") if not d.endswith(".csv") and "recommended" not in d]
    data = {
        "quantile": [],
        "alpha": [],
        "weight": [],
    ...
    for dir_name in dir_names:
        for col in cols:
            ...
            dir_path = os.path.join("results-bandwidth", dir_name)
    ...
    pd.DataFrame(data).to_csv("results-bandwidth/setting-table.csv")
    pd.DataFrame(results).to_csv("results-bandwidth/summary.csv", index=False)
    mean_vals = {k: np.asarray(v, dtype=np.float32) if k.startswith("n_evals") else v for k, v in mean_vals.items()}
    pd.DataFrame(mean_vals).to_csv("results-bandwidth/mean_vals.csv", index=False)
